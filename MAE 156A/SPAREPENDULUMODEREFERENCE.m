...
for i = 1:length(radii)
    ...
    for j = 1:length(t)-1
        ...
        k1 = dt * pendulumODE([theta(j); omega(j)], r);
        k2 = dt * pendulumODE([theta(j) + k1(1)/2; omega(j) + k1(2)/2], r);
        k3 = dt * pendulumODE([theta(j) + k2(1)/2; omega(j) + k2(2)/2], r);
        k4 = dt * pendulumODE([theta(j) + k3(1); omega(j) + k3(2)], r);
   ...
   %Mark the end of the oscillation
...
for i = 1:length(radii)
    ...
    legend('KE (Red)', 'PE (Blue)', 'TE (Black)', 'Location', 'southeast');
    grid on;
end


% Function to calculate the ODE 
function pendulumSOLN = pendulumODE(y, r)
    % Parameters
g = 9.81; % m/s^2
L = 0.154; % Length to center of mass in meters
m = 0.3123; %total mass in kg
I = 0.00837; % Intertia in kg*m^2
mu = 0.11; % Coefficient of friction
